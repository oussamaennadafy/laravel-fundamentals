...
@section('content')
    <main class="min-h-[calc(100vh-188px)]">
        <h1 class="text-3xl text-center my-7">edit post form</h1>

        <form class="lg:w-3/4 mx-auto p-5 bg-gray-100 rounded-md" method="POST" action="/posts/{{ $post->id }}">
            @csrf
            @method('PUT')
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input value="{{ $post->description }}" type="description" name="description" id="description"
                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " required />
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-6 group">
                    <input value="{{ $post->author }}" type="text" name="author" id="author"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " required />
                    <label for="author"
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input value="{{ $post->status }}" type="number" name="status" id="status"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " required />
                    <label for="status"
